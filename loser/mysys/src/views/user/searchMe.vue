...
<template>
  <div class="main">
    ...
    <el-card class="box-card" v-for="(item, index) in listdata" :key="index">
      ...
      <div>
        悬赏金：{{ item.reward || 0 }} 元
        <el-tag :type="item.payStatus === 1 ? 'success' : 'info'" style="margin-left: 10px;">
    ...
    <el-dialog :title="isAdd ? '增加' : '修改'" :visible.sync="isOccur" width="30%" :before-close="handleClose">
      <el-form ref="form" :model="form" label-width="80px">
        ...
        <!-- 新增：悬赏金输入框 -->
        <el-form-item label="悬赏金(元)">
          <el-input
              type="number"
              v-model="form.reward"
              min="0"
              step="0.01"
              placeholder="请输入悬赏金金额，0表示不设置">
          </el-input>
        </el-form-item>
...
export default {
  ...
  mounted() {
    // 加载用户自己的帖子时，会包含reward和payStatus字段（需后端支持）
